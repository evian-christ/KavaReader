...
struct PageStripView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            if isExpanded {
                naturalPageStrip
                    .transition(.asymmetric(insertion: .scale(scale: 0.8, anchor: .bottom).combined(with: .opacity),
    ...
    private var capsuleBackground: some View {
        ZStack {
            ...
            Capsule()
                .fill(LinearGradient(colors: [.white.opacity(isExpanded ? 0.25 : 0.2), .clear],
                                     startPoint: .top,
                                     endPoint: .bottom))
...
#Preview {
    ZStack {
        ...
        PageStripView(currentPage: .constant(5),
                      totalPages: 24,
                      onPageChange: { _ in })
            .padding(.bottom, 50)
    }
